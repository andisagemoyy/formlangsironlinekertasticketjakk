(() => {
  ...
  const state={shift:'',type:'R',editing:'',data:loadData(),routes:[],selected:new Set()};
  ...
  const selectionKey=(type,code)=>`${type}|${code}`;
  const allShiftOrders=()=>[...DATA[state.shift].R.map(code=>({code,type:'R'})),...DATA[state.shift].L.map(code=>({code,type:'L'}))];
  const selectedOrders=()=>allShiftOrders().filter(({code,type})=>state.selected.has(selectionKey(type,code)));
  ...
  function selectShift(shift){state.shift=shift;state.type='R';state.selected.clear();$('#home').hidden=true;$('#workspace').hidden=false;$('#shiftTitle').textContent='Dinas '+shift[0]+shift.slice(1).toLowerCase();loadDuty();render();window.scrollTo({top:0,behavior:'smooth'});}
  ...
  function render(){
    ...
    const savedType=codes.filter(code=>state.data.records[recordKey(code)]).length,selected=selectedOrders(),selectedType=codes.filter(code=>state.selected.has(selectionKey(state.type,code))).length,total=allCodes.length,pages=Math.ceil(selected.length/2),selectAll=$('#selectAllShift');
    $$('.type-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.type===state.type));
    $('#listTitle').textContent=state.type==='R'?'Langsir Rangkaian':'Lok Dinas';$('#listMeta').textContent=`${savedType} dari ${codes.length} sudah diisi · ${selectedType} dicentang`;
    $('#rCount').textContent=`${DATA[state.shift].R.length} KA`;$('#lCount').textContent=`${DATA[state.shift].L.length} lok`;
    $('#selectionHint').textContent=`${selected.length} dari ${total} form dipilih`;
    selectAll.checked=selected.length===total;selectAll.indeterminate=selected.length>0&&selected.length<total;
    $('#savedSummary').textContent=selected.length?`${selected.length} form dipilih · ${pages} lembar Letter`:'Belum ada form dipilih';
    $('#downloadExcel').disabled=$('#downloadExcelBottom').disabled=selected.length===0;
    $('#clearSelection').disabled=selected.length===0;
    $('#trainList').innerHTML=codes.map(code=>{const record=state.data.records[recordKey(code)],checked=state.selected.has(selectionKey(state.type,code));let desc='Jalur belum diisi — tetap masuk Excel';if(record){if(record.type==='L'&&record.input){desc=`1. JAKK VII → ${record.input.entry||'DAO kosong'}, gandeng ${record.input.coupling||'DAO kosong'}<br>2. ${record.input.coupling||'DAO kosong'} → ${record.input.return||'JAKK kosong'}`;}else desc=record.steps.map((s,i)=>`${i+1}. ${s.from_track||'Jalur kosong'} → ${s.to_track||'Jalur kosong'}`).join('<br>');}return `<article class="train-row ${record?'saved':''} ${checked?'selected':''}"><label class="form-check" title="Pilih ${esc(code)} untuk dicetak"><input type="checkbox" data-select-code="${esc(code)}" data-select-type="${state.type}" ${checked?'checked':''}><span></span></label><b class="train-code">${esc(code)}</b><div class="route-text">${record?'<b>Sudah disimpan</b>':''}${desc}</div><div class="row-actions"><button class="edit-route" data-code="${esc(code)}">${record?'Ubah jalur':'Isi jalur'}</button></div></article>`;}).join('');
  }
  ...
  async function exportOrders(codes=selectedOrders()){
    if(!codes.length)return toast('Checklist dulu form yang ingin dicetak.');
    saveDuty();const date=$('#serviceDate').value,duty=currentDuty();
    ...
    try{const bytes=await new window.XlsxTemplate(window.XLSX_TEMPLATES).export(prepared),blob=new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=prepared.length===1?`Form_Langsir_${prepared[0].code}_${date}.xlsx`:`Form_Langsir_${prepared.length}_Terpilih_${date}_${state.shift}.xlsx`;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);toast(`${prepared.length} form terpilih dibuat menjadi ${Math.ceil(prepared.length/2)} lembar Letter.`);}catch(error){console.error(error);toast('Excel gagal dibuat: '+error.message);}
  }
  ...
  $('#trainList').onchange=e=>{const input=e.target.closest('[data-select-code]');if(!input)return;const key=selectionKey(input.dataset.selectType,input.dataset.selectCode);if(input.checked)state.selected.add(key);else state.selected.delete(key);render();};
  $('#trainList').onclick=e=>{const b=e.target.closest('[data-code]');if(b)openRoute(b.dataset.code);};
  ...
  $('#selectAllShift').onchange=e=>{state.selected.clear();if(e.target.checked)for(const {code,type} of allShiftOrders())state.selected.add(selectionKey(type,code));render();};
