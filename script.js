(() => {
  ...
  function exportOrders(){
    saveDuty();const date=$('#serviceDate').value,duty=currentDuty(),codes=[...DATA[state.shift].R.map(code=>({code,type:'R'})),...DATA[state.shift].L.map(code=>({code,type:'L'}))];
    ...
    try{const bytes=new window.XlsTemplate(window.XLS_TEMPLATE).export(prepared),blob=new Blob([bytes],{type:'application/vnd.ms-excel'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`Form_Langsir_${date}_${state.shift}.xls`;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);toast(`${prepared.length} form dibuat dalam 1 sheet Excel memanjang.`);}catch(error){console.error(error);toast('Excel gagal dibuat: '+error.message);}
  }
